...
async def get_api_metrics():
    ...
    """
    try:
        # Read log file to extract metrics
        metrics = {
            "total_requests": 0,
        ...
        if os.path.exists("logs/api.log"):
            ...
            if api_calls:
                ...
                recent_calls = sorted(api_calls, key=lambda x: x["timestamp"], reverse=True)[:10]
    ...
    try:
        ...
        if os.path.exists("logs/api.log"):
            ...
            if ml_predictions:
                ...
                # Recent predictions
                recent_preds = sorted(ml_predictions, key=lambda x: x["timestamp"], reverse=True)[:10]
